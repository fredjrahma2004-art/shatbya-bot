...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("أهلاً بكِ يا رحما! البوت جاهز الآن. أرسلي لي ملف الـ PDF أو التسجيلات الصوتية للكتابين الآخرين هنا في المحادثة، وسأعطيكِ معرف كل ملف فوراً.")
...
async def get_file_id(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.message.document:
        ...
        await update.message.reply_text(f"هذا هو معرف التسجيل الصوتي (Voice):\n`{file_id}`", parse_mode="Markdown")
